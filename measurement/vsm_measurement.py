...
class AbstractMeasurement(ABC):
    ...
    def __init__(self,
                 ...
                 path: str,
                 lia_gpib: GPIBPathValue = 'GPIB0::7::INSTR',
                 magnet_gpib: GPIBPathValue = 'GPIB0::4::INSTR',
                 temperature_gpib: GPIBPathValue = 'GPIB0::12::INSTR',
                 **kwargs) -> None:
        """
    ...
    def _generate_file_name_prefix(self) -> str:
        return '{}_vsm_'.format(datetime.now().strftime('%X'))

    def _generate_plot_file_name_prefix(self, pair) -> str:
        return '{}_vsm_plot-{}-{}_'.format(datetime.now().strftime('%X'), pair[0], pair[1])
